...
Using CloudShell, clone the components of the Online Boutique from its repository to the Kubernetes cluster```git clone https://github.com/GoogleCloudPlatform/microservices-demo```
Open the microservices-demo directory and run the following command```kubectl apply -f ./release/kubernetes-manifests.yaml```
Wait for the pods to be ready. Run the following command periodically```kubectl get pods```
